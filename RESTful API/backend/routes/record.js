...
recordRoutes.route("/record/validAccount").post(async (req, res) => {
  try {
    ...
    let countValues = await db_connect.collection("records").countDocuments(query) 
    if(countValues > 0){
      return await res.status(400).json({message:"Duplicate password and email!!ssssssssssssss"}) //https://auth0.com/blog/how-to-implement-custom-error-responses-in-expressjs/
    }
    else{
      return await res.status(400).json({message:"Did not match password and email"}) //https://auth0.com/blog/how-to-implement-custom-error-responses-in-expressjs/
...
recordRoutes.route("/withdrawSavings/:email").post(async (req, res) => {
  try {
    ...
    return await res.status(400).json({message:"It Worked"})
  } catch (err) {
    throw err;
  }
});
...
recordRoutes.route("/withdrawChecking/:email").post(async (req, res) => {
  try {
    ...
    return await res.status(400).json({message:"IT did it!"}) //https://auth0.com/blog/how-to-implement-custom-error-responses-in-expressjs/
  } catch (err) {
    throw err;
  }
});
